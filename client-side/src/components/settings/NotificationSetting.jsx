...
const NotificationSetting = () => {
  ...
  const [history, setHistory] = useState([
    { id: 1, message: 'Account login detected', date: '2024-12-01', isNew: true },
    { id: 2, message: 'Password changed successfully', date: '2024-12-02', isNew: true },
  ]);
  ...
  const markAsRead = (id) => {
    setHistory(
      history.map((item) => (item.id === id ? { ...item, isNew: false } : item))
    );
  };
          ...
          <ul className="divide-y divide-gray-200">
            {history.map((item) => (
              <li key={item.id} className="py-4 flex justify-between items-center">
                <div>
                  <span
                    className={`text-gray-700 ${item.isNew ? 'font-bold' : ''}`}
                  >
                    {item.message}
                  </span>
